...
public class IntegerConverterTest extends TestCase
{
    ...
    public void testIntToWords()
    {
        for (int i = 1; i <= 999; ++i) {
            if (i < 10) {
               assertEquals(ONES[(i-1)], IntegerConverter.getAsWord(i));
            }
            else if (i % 10 == 0 && i < 100) {
                int num = i / 10;
                assertEquals(TENS[num - 1], IntegerConverter.getAsWord(i));
            }
            else if (i < 20) {
                int num = i % 10;
                assertEquals(ELEVEN_NINETEEN[num - 1], IntegerConverter.getAsWord(i));
            }
            else if (i < 99) {
                String expected = "";
                ...
                assertEquals(expected, IntegerConverter.getAsWord(i));
            }
            else if (i < 999) {
                String expected = "";
                int hundreds = i / 100;
